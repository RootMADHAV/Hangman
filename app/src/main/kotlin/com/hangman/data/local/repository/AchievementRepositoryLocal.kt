...
import com.hangman.data.local.entity.toEntity
import com.hangman.domain.model.Achievement
import com.hangman.domain.model.AchievementCatalog
...
class AchievementRepositoryLocal @Inject constructor(
    ...
    override suspend fun unlockAchievement(achievementId: String): Result<Unit> =
        try {
            ...
            // Only grant the token reward on the FIRST unlock; repeat evaluations of an
            // already-unlocked achievement must not pay out again.
     ...
     */
    private fun merge(
        entities: List<com.hangman.data.local.entity.AchievementEntity>
    ): List<Achievement> {
