    ...
    override suspend fun recordCompletion(won: Boolean): Result<DailyStreak> {
        return try {
            ...
            val updated = dataStore.edit { preferences ->
                if (preferences[LAST_COMPLETED_DATE] == todayKey) {
                    return@edit
                }

                // The streak survives only if the previous completion was yesterday;
                // any longer gap restarts the count at this one day.
            ...
            Result.success(updated.toStreak())
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
